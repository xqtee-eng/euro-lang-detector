...
TEMPLATES = [
    ...
    "Nije tako {easy} razumeti taj problem." if "{easy}" == "lako" else "Nije tako {easy} razumjeti taj problem.",
    "Mi {always} pijemo {coffee} pre posla." if "{always}" == "uvek" else "Mi {always} pijemo {coffee} prije posla.",
    "Ona {always} naručuje {coffee} u ovom kafiću.",
    "Zašto {always} moraš piti {coffee} tako kasno?",
    "Na trgu je bilo više od {thousand} ljudi.",
    "Ovaj projekat košta više od {thousand} evra." if "{thousand}" == "hiljadu" else "Ovaj projekat košta više od {thousand} eura.",
    "Celi {world} zna za taj događaj." if "{world}" == "svet" else "Cijeli {world} zna za taj događaj.",
    "Ovo je najbolji {world} koji možemo zamisliti.",
    "Mislim da mi {should_go} kući što pre." if "{world}" == "svet" else "Mislim da mi {should_go} kući što prije.",
    "Moj prijatelj kaže da je to {easy}.",
    "On {always} misli da je u pravu.",
    "Naš {world} se brzo menja." if "{world}" == "svet" else "Naš {world} se brzo mijenja.",
    ...
    "Mi {always} verujemo u to." if "{always}" == "uvek" else "Mi {always} vjerujemo u to.",
    "Misliš da je to {easy}? Nije nimalo {easy}.",
    "{coffee_nom} je spremna, možemo da je pijemo." if "{always}" == "uvek" else "{coffee_nom} je spremna, možemo je piti."
]
...
def generate_sentences(lang_code, count):
    ...
    prefixes = ["Znaš da ", "Kažu da ", "Možda ", "Naravno, ", "Sigurno ", "Vidiš, ", "Iskreno, ", "Danas ", "Juče ", "Jucer ", "Sutra ", "Ujutro ", "Opet ", "Mislim da ", "Zato ", "Sada ", "Zaista, ", "Verovatno " if lang_code == "sr" else "Vjerojatno "]
    suffixes = [" zar ne?", " danas.", " odmah.", " uvek." if lang_code == "sr" else " uvijek.", " zar ne misliš tako?", " to je sigurno.", " bez sumnje."]
    ...
    while len(generated) < count and attempts < max_attempts:
        ...
        if lang_code == "sr":
            sentence = sentence.replace("razumjeti", "razumeti").replace("Jucer", "Juce").replace("vjerujemo", "verujemo")
        else:
            sentence = sentence.replace("razumeti", "razumjeti").replace("Juce", "Jucer").replace("verujemo", "vjerujemo")
...
def main():
    ...
    for lang in ["bs", "hr", "sr"]:
        ...
        actual_train_count = min(train_count, len(all_sentences) - 10) # leave at least 10 for bench
